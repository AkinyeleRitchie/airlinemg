...
# Database setup
def setup_database():
    conn = sqlite3.connect('airline_manage.db')  # Database name updated
    cursor = conn.cursor()
    cursor.execute('''
...
def show_login_page():
    ...
    def login():
        ...
        if result:
            ...
            open_main_page()  # Open the main.py page
        else:
...
# Function to open main.py page
def open_main_page():
    # This will open main.py as a separate process.
    # Ensure that main.py is in the same directory as this script or provide its full path.
    subprocess.Popen(["python", "main.py"])  # On Windows, you may need to use "python" or "python3"
